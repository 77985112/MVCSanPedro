...
if (isset($_GET['id'])) {
    $idGrupo = $_GET['id'];
    $datosGrupo = $contG->mostrarGrupoCat($idGrupo);

    if ($datosGrupo) {
        $grupo = $datosGrupo['grupo'];
        $celGrupo = $datosGrupo['celebrantes'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $color = $_POST['Color_Grupo'];
    $santo = $_POST['Santo_Grupo'];
    $frase = $_POST['Frase_Grupo'];
    $imagen = $grupo['Imagen_Grupo'];

    $contG->actualizarGrupo($idGrupo, $nombre, $color, $santo, $imagen, $frase);
    header("Location: GrupoEsp.php?id=" . $Catequista['IdGrupo']);
}
                                ...
                                <div class="card overflow-hidden">
                                    ...
                                    <div class="card-body pt-0">
                                        ...
                                        <ul class="nav nav-link-secondary flex-column fw-bold gap-2">
                                            ...
                                            <li class="nav-item">
                                                <a class="nav-link active" href="GrupoEsp.php?id=<?php echo $Catequista['IdGrupo']; ?>"> <img class="me-2 h-20px fa-fw" src="../assets/images/icon/contrato.png" alt=""><span>Grupo </span></a>
                                            </li>
                                            <li class="nav-item">
                ...
                <div class="col-md-8 col-lg-6 vstack gap-4">
                    ...
                    <div class="card h-100">
                        ...
                        <div class="card-header d-sm-flex align-items-center text-center justify-content-sm-between border-0 pb-0">
                            <h3 class="card-title">Información del <strong>Grupo <?php echo $grupo['NombreGrupo']; ?></strong></h3>
                            <?php if ($Catequista['RolCat'] == "Titular") { ?>
                                <div class="mb-3 d-flex align-items-end">
                                    <button type="button" class="btn btn-primary-soft me-2" data-bs-toggle="modal" data-bs-target="#MostrarModal"><i class="bi bi-pencil-square"></i></button>
                                </div>
                            <?php } ?>
                        </div>
                        <div class="card-body col-lg-12">
                            <h3 class="mb-0">Información de Catequisandos</h3>
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    <?php foreach ($celGrupo as $celG): ?>
                                        <tr>
                                            <td><?php echo $celG['CiCel']; ?></td>
                                            <td><?php echo $celG['NombreCompleto']; ?></td>
                                            <td><?php echo $celG['DescriptItem']; ?></td>
                                            <td><?php echo $celG['Estado']; ?></td>
                                            <td>
                                                <form method="post" action="">
                                                    <input type="hidden" name="ACCIns" value="<?= $celG['IdInscripcion'] ?>">
                                                    <button type="submit" name="ver_apoderado_padrino" class="btn btn-primary-soft"><i class="bi bi-file-earmark-break-fill"></i></button>
                                                </form>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addModalLabel">Modificar Grupo</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="GrupoEsp.php?id=<?php echo $grupo['IdGrupo']; ?>" method="POST" enctype="multipart/form-data">
                        <div class="card-header border-0 pb-0 d-md-flex">
                            <div class="card-body col-lg-8 me-5">
                                <ul class="list-group">
                                    <li class="list-group-item d-md-flex justify-content-between align-items-center">
                                        <div class="me-md-3">
                                            <h6 class="mb-0">Imagen de Santo</h6>
                                            <img id="imagenPreviewFondo" class="h-200px rounded" src="<?php echo ($grupo['Imagen_Grupo'] == 'Anonimo' || empty($grupo['Imagen_Grupo'])) ? '../assets/images/Santo/SanPedro.png' : "../assets/images/Santo/{$grupo['Imagen_Grupo']}"; ?>" alt="" style="background-size: cover; background-repeat: no-repeat; width: 100%;">
                                            <p class="small mb-0"><?php echo ($grupo['Imagen_Grupo'] == 'Anonimo' || empty($grupo['Imagen_Grupo'])) ? 'Fondo Genérico' : 'Fondo Actual'; ?></p>
                                        </div>
                                        <div class="me-md-3">
                                            <h6>Nueva Imagen</h6>
                                    ...
                                    <input type="file" id="inputImagenGrupo" name="Imagen_Grupo" accept="image/*" class="btn btn-secondary-soft me-2" onchange="mostrarImagen(event)">
                                </ul>
                            </div>
                            <div class="card-body col-lg-4">
                                <h3 class="py-3 col-lg-12">Más Información</h3>
                                <div class="col-lg-8">
                                    <label class="form-label">Santo del Grupo</label>
                                    <input type="text" class="form-control" name="Santo_Grupo" value="<?php echo $grupo['Santo_Grupo']; ?>">
                                </div>
                                <div class="col-lg-4">
                                    <label class="form-label">Color actual</label>
                                    <input type="color" class="form-control" name="Color_Grupo" value="<?php echo $grupo['Color_Grupo']; ?>">
                                </div>
                                <div class="col-lg-12">
                                    <label class="form-label">Frase del Grupo</label>
                                    <input type="text" class="form-control" name="Frase_Grupo" value="<?php echo $grupo['Frase_Santo']; ?>">
                                </div>
                            </div>
                        </div>
                        <input type="hidden" name="IdGrupo" value="<?php echo $grupo['IdGrupo']; ?>">
                        <input type="hidden" name="NombreGrupo" value="<?php echo $grupo['NombreGrupo']; ?>">
                        <br>
                        <button type="submit" class="btn btn-primary-soft mb-0 w-50">Actualizar datos de Grupo</button>
                    </form>
                </div>
            </div>
        </div>
