...
$catequistas = $controller->obtenerCatesInactivos($codIns);
...
class CertificadoPDF extends FPDF {
    public function generarPDF($catequistas) {
        if (!empty($catequistas)) {
            ...
            $this->Cell(0, 10, 'Reporte de Catequistas "Inactivos" de la '.iconv('UTF-8', 'Windows-1252//IGNORE', 'gestión ') . date('Y'), 0, 1, 'C');
            $this->Ln(5);
            ...
            foreach ($catequistas as $fila) {
                ...
                $this->Cell(15, 5, iconv('UTF-8', 'Windows-1252//IGNORE', $fila['ApoyoEst']), 1);
        ...
        else
        {
            ...
            $this->Ln(5);
            $this->SetFont('Arial', 'B', 10);
            $this->Cell(50, 10, 'No hay Catequistas Inactivos...', 0);
            $this->Ln();
            $this->Output();
        }
    }
