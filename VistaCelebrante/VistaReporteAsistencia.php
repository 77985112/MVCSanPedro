...
$celebrantes = $controller->obtenerAsistencias($ins);
...
class CertificadoPDF extends FPDF
{
    public function generarPDF($celebrantes, $catequistas, $nombreCelebrante, $apPaterno, $apMaterno, $sacramento)
    {
        ...
        $this->Cell(0, 10, "Mi estado en Asistencia a Catequesis " . date('Y'), 0, 1, 'C');
        $this->Ln(0);
        ...
        if (!empty($celebrantes)) {
            ...
            $this->Cell(50, 5, 'Fecha de Asistencia', 1, 0, 'C');
            $this->Cell(70, 5, 'Detalle de Asistencia', 1, 0, 'C');
            $this->Cell(50, 5, 'Estado de Asistencia', 1, 0, 'C');
            $this->Ln();

            foreach ($celebrantes as $celeb) {
                $this->SetFont('Arial', '', 10);
                $this->Cell(50, 5, iconv('UTF-8', 'Windows-1252//IGNORE', $celeb['FechaAsisConf']), 1);
                $this->Cell(70, 5, iconv('UTF-8', 'Windows-1252//IGNORE', $celeb['DetalleAsis']), 1);
                $this->Cell(50, 5, iconv('UTF-8', 'Windows-1252//IGNORE', $celeb['TipoAsis']), 1);
                $this->Ln();
            }
        } else {
