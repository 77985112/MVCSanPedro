...
$CelGr = $Celebrante['IdGrupo'];
$idIns = $Celebrante['IdInscripcion'];

$apoderadosModelo = new Modelo();

$apoderadosYPadrinos = $apoderadosModelo->obtenerApoderadosYPadrinos($idIns);
$documentosPresentados = $apoderadosModelo->obtenerDocumentosPresentados($idIns);

$diassemana = array("Domingo", "Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado");
$meses = array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");

require_once '../Modelo/modeloCatAdmin.php';
$catequistaModel = new Catequista();

$catequistas = $catequistaModel->CatequistasDelGrupoCel($CelGr);
    ...
    <div class="container">
      <div class="row pt-3">
        <div class="col-lg-8 vstack gap-4">
          <div class="card">
            <div class="h-200px rounded-top" style="background-image:url(<?php echo ($Celebrante['FondoCel'] == 'Ninguno' || empty($Celebrante['FondoCel'])) ? '../assets/images/fondo/Ninguno.jpg' : "../assets/images/fondo/{$Celebrante['FondoCel']}"; ?>); background-position: center; background-size: cover; background-repeat: no-repeat;"></div>

            <div class="card-body py-0">
              <div class="d-sm-flex align-items-start text-center text-sm-start">
                <div>
                  <div class="avatar avatar-xxl mt-n5 mb-3">
                    <img class="avatar-img rounded-circle border border-white border-3" src="<?php echo ($Celebrante['PerfilCel'] == 'Ninguno' || empty($Celebrante['PerfilCel'])) ? '../assets/images/avatar/Ninguno.png' : "../assets/images/avatar/{$Celebrante['PerfilCel']}"; ?>" alt="">
                  </div>
                </div>
                <div class="ms-sm-4 mt-sm-3">
                  <h1 class="mb-0 h5"><?php echo $Celebrante['Nombre'] . " " . $Celebrante['ApPaterno'] . " " . $Celebrante['ApMaterno']; ?></h1>
                  <p>Catequisando "<strong>San Pedro de Sacaba</strong>"</p>
                </div>

              </div>
              <ul class="list-inline mb-0 text-center text-sm-start mt-3 mt-sm-0">
                <li class="list-inline-item"><i class="bi bi-briefcase me-1"></i><?php echo $Celebrante['Sacramento']; ?></li>
                <li class="list-inline-item"><i class="bi bi-geo-alt me-1"></i>Sacaba - Chapare</li>
                <li class="list-inline-item"><i class="bi bi-person-rolodex me-1"></i> Preparación <strong><?php echo $Celebrante['DescripItem']; ?></strong></li>
              </ul>
            </div><br>
          </div>
          ...
          <div class="card card-body">
            ...
            <?php if ($Celebrante['EstadoCel'] == "Activo") : ?>


              <h3>Apoderados y Padrinos</h3>
              <table class="table table-hover">
                ...
                <tbody>
                  <?php if (!empty($apoderadosYPadrinos)) : ?>
                    <?php foreach ($apoderadosYPadrinos as $familiar) : ?>
                      <tr>
                        <td><?= htmlspecialchars($familiar['RolFam']) ?></td>
                        <td><strong><?= htmlspecialchars($familiar['Nombre'] . ' ' . $familiar['ApPaterno'] . ' ' . $familiar['ApMaterno']) ?></strong></td>
                        <td><?= htmlspecialchars($familiar['Parentesco']) ?></td>
                      </tr>
                    <?php endforeach; ?>
                  <?php else : ?>
                    <tr>
                      <td colspan="5">No se han registrado apoderados ni padrinos.</td>
                    </tr>
                  <?php endif; ?>
                </tbody>
              </table>
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <?php if (!empty($documentosPresentados)) : ?>
                    <?php foreach ($documentosPresentados as $documento) : ?>
                      <tr>
                        <td><?= htmlspecialchars($documento['NumeroDoc']) ?></td>
                        <td><?= htmlspecialchars($documento['DetalleDoc']) ?></td>
                        <td><?= htmlspecialchars($documento['ParroquiaDoc']) ?></td>
                        <td><?= htmlspecialchars($documento['FechaPres']) ?></td>
                      </tr>
                    <?php endforeach; ?>
                  <?php else : ?>
                    <tr>
                      <td colspan="7">No se han presentado documentos.</td>
                    </tr>
                  <?php endif; ?>
                </tbody>
              </table>
            ...
            <?php elseif ($Celebrante['EstadoCel'] == "Inactivo") : ?>
              ...
              <div class="card">
                ...
                <div class="card-body position-relative pt-0">
                  <ul class="list-unstyled mt-3 mb-0">
                    <li class="mb-2"> <i class="bi bi-arrow-through-heart fa-fw pe-1"></i> Estado: <strong> <?php echo $Celebrante['Estado_per']; ?> </strong> </li>

                    <li class="mb-2" <?php if ($Celebrante['EstadoCel'] == "Activo") : ?>style="color:limegreen;" <?php elseif ($Celebrante['EstadoCel'] == "Inactivo") : ?>style="color:Red;" <?php endif; ?>> <i class="bi bi-broadcast"></i> Estado: <strong> <?php echo $Celebrante['EstadoCel']; ?> </strong> </li>

                    <li> <i class="bi bi-telephone fa-fw pe-1"></i> Contacto: <strong> <?php echo $Celebrante['Contacto']; ?> </strong> </li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div class="col-md-6 col-lg-12">
              <div class="card">
                <div class="card-header d-sm-flex justify-content-between align-items-center border-0">
                  <h5 class="card-title">Mis Catequistas </h5>
                </div>
                <div class="card-body position-relative pt-0">
                  <div class="row g-3">

                    <?php if (!empty($catequistas)): ?>
                      <?php foreach ($catequistas as $cates): ?>
                        <div class="col-6">
                          <div class="card shadow-none text-center h-100">
                            <div class="card-body p-2 pb-0">
                              <input type="hidden" value="<?php echo htmlspecialchars($cates['CiCat']); ?>">
                              <div class="avatar avatar-xl">
                                <img class="avatar-img rounded-circle" src="<?php echo ($cates['ImagenCat'] == 'Ninguno' || empty($cates['ImagenCat'])) ? '../assets/images/avatar/Ninguno.png' : "../assets/images/avatar/{$cates['ImagenCat']}"; ?>" alt="">
                              </div>
                              <h6 class="card-title mb-1 mt-3"> <?php echo htmlspecialchars($cates['NombreCompleto']); ?></h6>
                              <p class="mb-0 small lh-sm"><?php echo htmlspecialchars($cates['RolCat']); ?></p>
                              <br>
                            </div>
                          </div>
                        </div>
                      <?php endforeach; ?>
                    <?php else: ?>
                      <p>No hay catequistas en este grupo.</p>
                    <?php endif; ?>
                  </div>
                </div>
              </div>
